...
static int              get_archive_data(int, int),
                        send_new_message(char *, time_t, unsigned short, int),
                        get_file(char *, char *),
                        rdu(char *, off_t *);
...
resend_files(int no_selected, int *select_list)
{
   ...
   p_msg_name = dest_dir;
   p_msg_name += sprintf(dest_dir, "%s%s/",
                         p_work_dir, AFD_FILE_DIR);

   /* Get the fsa_id and no of host of the FSA */
   if (fsa_attach() < 0)
   {
      (void)xrec(toplevel_w, FATAL_DIALOG, "Failed to attach to FSA. (%s %d)",
                 __FILE__, __LINE__);
      return;
   }
   ...
   while (to_do > 0)
   {
               ...
               if ((select_done % max_copied_files) == 0)
               {
                  ...
                  if (select_done != 0)
                  {
                     ...
                     if (send_new_message(p_msg_name, creation_time,
                                          unique_number, current_job_id) < 0)
                     {
                        (void)xrec(toplevel_w, FATAL_DIALOG,
                  ...
                  *p_msg_name = '\0';
                  if (create_name(dest_dir, job_priority, creation_time,
                                  current_job_id, &unique_number,
                                  p_msg_name) < 0)
                  {
                     (void)xrec(toplevel_w, FATAL_DIALOG,
      ...
      if ((select_done > 0) && ((select_done % max_copied_files) != 0))
      {
         ...
         if (send_new_message(p_msg_name, creation_time, unique_number,
                              last_job_id) < 0)
         {
            (void)xrec(toplevel_w, FATAL_DIALOG,
                       "Failed to create message : (%s %d)",
   ...
   {
      (void)xrec(toplevel_w, WARN_DIALOG, "close() error : %s (%s %d)",
                 strerror(errno), __FILE__, __LINE__);
   }
                 ...
                 int            job_id)
{
   int    fd,
          offset;
   size_t msg_fifo_buf_size = 1 + sizeof(time_t) + sizeof(unsigned short) + sizeof(int);
   char   *fifo_buffer,
   ...
   {
      (void)xrec(toplevel_w, ERROR_DIALOG, "Could not open %s : %s (%s %d)",
   ...
   *(int *)(fifo_buffer + offset) = job_id;
   *(unsigned short *)(fifo_buffer + offset + offset) = unique_number;
   *(char *)(fifo_buffer + offset + offset + sizeof(unsigned short)) = p_msg_name[0];

   /* Send the message */
   if (write(fd, fifo_buffer, msg_fifo_buf_size) != msg_fifo_buf_size)
   {
      (void)xrec(toplevel_w, ERROR_DIALOG, "Could not write to %s : %s (%s %d)",
                 msg_fifo, strerror(errno), __FILE__, __LINE__);
      free(fifo_buffer);
      return(INCORRECT);
   }
   ...
   if (close(fd) == -1)
   {
      (void)xrec(toplevel_w, WARN_DIALOG, "Failed to close() %s : %s (%s %d)",
                 msg_fifo, strerror(errno), __FILE__, __LINE__);
   }
   free(fifo_buffer);
...
write_fsa(int add)
{
   ...
   files_to_send = rdu(dest_dir, &file_size_to_send);
      ...
      if (*ptr == '@')
      {
         ...
         *ptr = tmp_char;
         if ((position = get_host_position(fsa, truncated_hostname, no_of_hosts)) != INCORRECT)
         {
            ...
            unlock_region(fsa_fd, (char *)&fsa[position].total_file_counter - (char *)fsa);
         }
...
/*-------------------------------- rdu() --------------------------------*/
/*                                 -----                                 */
/* Description: Counts all files and the summary of their size in the    */
/*              directory 'dirname'.                                     */
/*-----------------------------------------------------------------------*/
static int
rdu(char *dirname, off_t *file_size)
{
   int            no_of_files = 0;
   char           *ptr = dirname;
   DIR            *p_dir;
   ...
   *p_dest_dir_end = '\0';
   ...
   {
      (void)xrec(toplevel_w, ERROR_DIALOG, "opendir() %s error : %s (%s %d)",
   ...
   while ((p_struct_dir = readdir(p_dir)) != NULL)
   {
      if ((p_struct_dir->d_name[0] == '.') &&
          ((p_struct_dir->d_name[1] == '\0') ||
           (p_struct_dir->d_name[1] == '.')))
      {
         continue;
      }
   ...
   if (closedir(p_dir) == -1)
   {
      (void)xrec(toplevel_w, WARN_DIALOG, "closedir() error : %s (%s %d)",
                 strerror(errno), __FILE__, __LINE__);
   }
