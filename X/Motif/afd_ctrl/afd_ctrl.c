 ...
 **
 ** SYNOPSIS
 **   afd_ctrl [--version][-w <AFD working directory>][-no_input][numeric font]
 **
 ** DESCRIPTION
 **
 ** RETURN VALUES
...
init_afd_ctrl(int *argc, char *argv[], char *window_title)
{
   ...
   {
      (void)rec(STDERR_FILENO, FATAL_SIGN,
   ...
   memcpy(prev_afd_status.receive_log_fifo, p_afd_status->receive_log_fifo, LOG_FIFO_SIZE + 1);
...
init_popup_menu(Widget line_window_w)
{
       ...
       (traceroute_cmd != NULL))
   {
      ...
      if (acp.debug != NO_PERMISSION)
      {
         ...
         XtSetArg(args[argcount], XmNfontList, fontlist); argcount++;
         pw[5] = XmCreatePushButton(popupmenu, "Debug", args, argcount);
