...
check_tv_status(Widget w)
{
   ...
   for (i = 0; i < no_of_jobs_selected; i++)
   {
       ...
       * ========
       */
      if (jd[i].priority[0] != fsa[jd[i].fsa_no].job_status[jd[i].job_no].unique_name[0])
      {
         if (x == -1)
         {
            tv_locate_xy(i, &x, &y);
         ...
         jd[i].priority[0] = fsa[jd[i].fsa_no].job_status[jd[i].job_no].unique_name[0];
         jd[i].priority[1] = fsa[jd[i].fsa_no].job_status[jd[i].job_no].unique_name[1];
         draw_tv_priority(i, x, y);
#ifdef _WITH_MAP_SUPPORT
         if ((fsa[jd[i].fsa_no].job_status[jd[i].job_no].connect_status != MAP_ACTIVE) &&
             (jd[i].priority[0] == '\0'))
#else
         if (jd[i].priority[0] == '\0')
#endif
         {
