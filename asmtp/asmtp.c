 ...
 **
 ** SYNOPSIS
 **   asmtp [options] -h <host name | IP number> files
 **
 **   options
...
main(int argc, char *argv[])
{
   ...
   else
   {
      if (db.verbose == YES)
      {
         trans_log(INFO_SIGN, __FILE__, __LINE__, "Connected.");
      }
   }
   ...
   for (files_send = 0; files_send < db.no_of_files; files_send++)
   {
      ...
      if ((fd = open(db.filename[files_send], O_RDONLY)) < 0)
      {
         ...
         else
         {
            if (db.verbose == YES)
            {
               trans_log(INFO_SIGN, __FILE__, __LINE__, "Closing data mode.");
            }
         }
         continue;
      }
   ...
   trans_log(INFO_SIGN, __FILE__, __LINE__, "%d Bytes send in %d file(s).",
             file_size_done, no_of_files_done);
   msg_str[0] = '\0';
   free(buffer);
