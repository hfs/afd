 ...
 **
 ** SYNOPSIS
 **   aftp [options] -h <host name | IP number> files
 **
 **   options
...
main(int argc, char *argv[])
{
   ...
   if ((status = ftp_connect(db.hostname, db.port)) != SUCCESS)
   {
      if (db.verbose == YES)
      {
         if (timeout_flag == OFF)
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Could not connect to %s (%d). (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, db.hostname,
                      status, __FILE__, __LINE__);
         }
         else
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Could not connect to %s due to timeout. (%s %d)\n",
      ...
      if (timeout_flag == OFF)
      {
         (void)rec(transfer_log_fd, ERROR_SIGN,
                   "%-*s: Failed to connect (%d). (%s %d)\n",
                   MAX_HOSTNAME_LENGTH, tr_hostname,
                   status, __FILE__, __LINE__);
         if (status != INCORRECT)
         {
            (void)rec(transfer_log_fd, ERROR_SIGN, "%-*s: %s\n",
   ...
   if ((status = ftp_user(db.user)) != SUCCESS)
   {
      if (db.verbose == YES)
      {
         ...
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to send user %s (%d). (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, db.user,
                      status, __FILE__, __LINE__);
            (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
         }
         ...
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to send user %s due to timeout. (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname,
   ...
   if ((status = ftp_pass(db.password)) != SUCCESS)
   {
      if (db.verbose == YES)
      {
         ...
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to send password for user %s (%d). (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname,
                      db.user, status, __FILE__, __LINE__);
            (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
         }
         else
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to send password for user %s due to timeout. (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname,
                      db.user, __FILE__, __LINE__);
         }
      }
   ...
   if ((status = ftp_type(db.transfer_mode)) != SUCCESS)
   {
      if (db.verbose == YES)
      {
         ...
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to set transfer mode to %c (%d). (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname,
                      db.transfer_mode, status, __FILE__, __LINE__);
            (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
         }
         else
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to set transfer mode to %c due to timeout. (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname,
   ...
   if (db.target_dir[0] != '\0')
   {
      if ((status = ftp_cd(db.target_dir)) != SUCCESS)
      {
         if (db.verbose == YES)
         {
            ...
            {
               (void)rec(trans_db_log_fd, ERROR_SIGN,
                         "%-*s: Failed to change directory to %s (%d). (%s %d)\n",
                         MAX_HOSTNAME_LENGTH, tr_hostname,
                         db.target_dir, status, __FILE__, __LINE__);
               (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                         MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
            }
            else
            {
               (void)rec(trans_db_log_fd, ERROR_SIGN,
                         "%-*s: Failed to change directory to %s due to timeout. (%s %d)\n",
                         MAX_HOSTNAME_LENGTH, tr_hostname,
                         db.target_dir, __FILE__, __LINE__);
            }
         }
         if (timeout_flag == OFF)
         {
   ...
   for (files_send = 0; files_send < db.no_of_files; files_send++)
   {
      ...
      if ((db.file_size_offset > -1) &&
          (db.restart_file[0] != '\0') &&
          (strcmp(db.restart_file, initial_filename) == 0))
      {
         if ((status = ftp_list(initial_filename, line_buffer)) != SUCCESS)
         {
            if (db.verbose == YES)
            {
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to list remote file %s (%d). (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname,
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to list remote file %s due to timeout. (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname,
      ...
      if ((status = ftp_data(initial_filename, append_offset)) != SUCCESS)
      {
         if (db.verbose == YES)
         {
            ...
            {
               (void)rec(trans_db_log_fd, ERROR_SIGN,
                         "%-*s: Failed to open remote file %s (%d). (%s %d)\n",
                         MAX_HOSTNAME_LENGTH, tr_hostname,
                         initial_filename, status, __FILE__, __LINE__);
               (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                         MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
            ...
            {
               (void)rec(trans_db_log_fd, ERROR_SIGN,
                         "%-*s: Failed to open remote file %s due to timeout. (%s %d)\n",
                         MAX_HOSTNAME_LENGTH, tr_hostname,
                         initial_filename, __FILE__, __LINE__);
            }
         }
         ...
         if (timeout_flag == OFF)
         {
            ...
            /*
             * If another error, ie not a remote error, has occured
             * it is of no intrest what the remote sever has to say.
             */
            if (status != -1)
            {
               (void)rec(transfer_log_fd, ERROR_SIGN, "%-*s: %s\n",
      ...
      for (;;)
      {
         for (j = 0; j < loops; j++)
         {
            ...
            if (ftp_write(buffer, ascii_buffer, db.blocksize) < 0)
            {
               /*
         ...
         if (rest > 0)
         {
            ...
            if (ftp_write(buffer, ascii_buffer, rest) < 0)
            {
               /*
                * It could be that we have received a SIGPIPE
                * signal. If this is the case there might be data
      ...
      if ((status = ftp_close_data()) != SUCCESS)
      {
         ...
         if ((local_file_size > 0) || (timeout_flag == ON))
         {
            if (db.verbose == YES)
            {
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to close remote file %s (%d). (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname,
                            initial_filename, status, __FILE__, __LINE__);
                  (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to close remote file %s due to timeout. (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname,
                            initial_filename, __FILE__, __LINE__);
               }
            }
         ...
         else
         {
            ...
            {
               (void)rec(trans_db_log_fd, WARN_SIGN,
                         "%-*s: Failed to close remote file %s (%d). Ignoring since file size is %d. (%s %d)\n",
                         MAX_HOSTNAME_LENGTH, tr_hostname,
            ...
            (void)rec(transfer_log_fd, WARN_SIGN,
                      "%-*s: Failed to close remote file %s. Ignoring since file size is %d. (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname,
                      initial_filename, local_file_size, __FILE__, __LINE__);
         }
      }
      else
      {
      ...
      if (db.lock == DOT)
      {
         if ((status = ftp_move(initial_filename, final_filename)) != SUCCESS)
         {
            if (db.verbose == YES)
            {
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to move remote file %s to %s (%d). (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, initial_filename,
                            final_filename, status, __FILE__, __LINE__);
                  (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
               }
               else
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to move remote file %s to %s due to timeout. (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, initial_filename,
                            final_filename, __FILE__, __LINE__);
               }
            }
      ...
      if ((db.lock == READY_A_FILE) || (db.lock == READY_B_FILE))
      {
         ...
         if ((status = ftp_data(ready_file_name, append_offset)) != SUCCESS)
         {
            if (db.verbose == YES)
            {
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to open remote ready file %s (%d). (%s %d)\n",
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to open remote ready file %s due to timeout. (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, ready_file_name,
                            __FILE__, __LINE__);
               }
            }
         ...
         if (ftp_write(ready_file_buffer, NULL, rdy_length) < 0)
         {
            /*
             * It could be that we have received a SIGPIPE
             * signal. If this is the case there might be data
            ...
            if (db.verbose == YES)
            {
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to write to remote ready file %s (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, ready_file_name,
                            __FILE__, __LINE__);
                  (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to write to remote ready file %s due to timeout. (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, ready_file_name,
                            __FILE__, __LINE__);
               }
            }
         ...
         if ((status = ftp_close_data()) != SUCCESS)
         {
            if (db.verbose == YES)
            {
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to close remote ready file %s (%d). (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, ready_file_name,
                            status, __FILE__, __LINE__);
                  (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to close remote ready file %s due to timeout. (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, ready_file_name,
                            __FILE__, __LINE__);
               }
            }
         ...
         if ((status = ftp_move(ready_file_name, &ready_file_name[1])) != SUCCESS)
         {
            if (db.verbose == YES)
            {
               ...
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to move remote ready file %s to %s (%d). (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, ready_file_name,
                            &ready_file_name[1], status, __FILE__, __LINE__);
                  (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
               }
               else
               {
                  (void)rec(trans_db_log_fd, ERROR_SIGN,
                            "%-*s: Failed to move remote ready file %s to %s due to timeout. (%s %d)\n",
                            MAX_HOSTNAME_LENGTH, tr_hostname, ready_file_name,
   ...
   (void)sprintf(line_buffer, "%-*s: %d Bytes send in %d file(s).",
                 MAX_HOSTNAME_LENGTH, tr_hostname,
                 file_size_done,
                 files_send);
   ...
   if ((status = ftp_quit()) != SUCCESS)
   {
      if (db.verbose == YES)
      {
         ...
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to disconnect from remote host. (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, __FILE__, __LINE__);
            (void)rec(trans_db_log_fd, INFO_SIGN, "%-*s: %s\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
         }
         ...
         {
            (void)rec(trans_db_log_fd, ERROR_SIGN,
                      "%-*s: Failed to disconnect from remote host due to timeout. (%s %d)\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, __FILE__, __LINE__);
         }
      }
       ...
       * not neccessary to indicate an error in the status display.
      ...
      {
         (void)rec(transfer_log_fd, WARN_SIGN,
                   "%-*s: Failed to log out (%d). (%s %d)\n",
                   MAX_HOSTNAME_LENGTH, tr_hostname, status, __FILE__, __LINE__);
         if (status != INCORRECT)
         {
            (void)rec(transfer_log_fd, WARN_SIGN, "%-*s: %s\n",
                      MAX_HOSTNAME_LENGTH, tr_hostname, msg_str);
         }
      }
      else
      {
         (void)rec(transfer_log_fd, WARN_SIGN,
                   "%-*s: Failed to log out due to timeout. (%s %d)\n",
                   MAX_HOSTNAME_LENGTH, tr_hostname, __FILE__, __LINE__);
      }
   }
   else
