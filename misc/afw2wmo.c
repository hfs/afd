...
afw2wmo(char *msg, int *msg_length, char **wmo_buffer, char *msg_name)
{
   ...
   *msg_length -= garbage;
   ...
   /* Check for TT */
   if ((isalpha(*read_ptr) == 0) || (isalpha(*(read_ptr + 1) == 0)))
   {
...
*/
   ...
   *(write_ptr + 1) = '\n';
   ...
   else
   {
      ...
      /* Check for ii */
      if (isdigit(*read_ptr) && isdigit(*(read_ptr + 1)))
      {
         *write_ptr = *read_ptr;
...
*/
   ...
   *write_ptr = ' ';
   ...
   if (isdigit(*read_ptr))
   {
      ...
      if ((isdigit(*(read_ptr + 1)) == 0) || (isdigit(*(read_ptr + 2)) == 0) ||
          (isdigit(*(read_ptr + 3)) == 0) || (isdigit(*(read_ptr + 4)) == 0))
      {
...
*/
   ...
   else if (isalpha(*read_ptr))
        {
           int is_slash_slash = NO;

           if ((isalpha(*(read_ptr + 1)) == 0) ||
               (isalpha(*(read_ptr + 2)) == 0) ||
               (isalpha(*(read_ptr + 3)) == 0))
           {
