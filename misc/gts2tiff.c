...
gts2tiff(char *path, char *filename)
{
   ...
   {
#ifdef _NO_MMAP
      if ((src = mmap_emu(0, stat_buf.st_size, PROT_READ, (MAP_FILE | MAP_SHARED), from, 0)) == (caddr_t) -1)
#else
...
#ifdef _NO_MMAP
      if ((dst = mmap_emu(0, tiff_file_size, (PROT_READ | PROT_WRITE), (MAP_FILE |  MAP_SHARED), to, 0)) == (caddr_t) -1)
#else
      if ((dst = mmap(0, tiff_file_size, (PROT_READ | PROT_WRITE), (MAP_FILE |  MAP_SHARED), fdout, 0)) == (caddr_t) -1)
#endif
      {
#ifdef _NO_MMAP
         if (munmap_emu(src) < 0)
