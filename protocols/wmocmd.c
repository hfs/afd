...
wmo_connect(char *hostname, int port)
{
   ...
   while (connect(sock_fd, (struct sockaddr *) &sin, sizeof(sin)) < 0)
   {
      ...
      if ((sock_fd = socket(sin.sin_family, SOCK_STREAM, IPPROTO_TCP)) < 0)
      {
         trans_log(ERROR_SIGN, __FILE__, __LINE__, "socket() error : %s",
                   strerror(errno));
         (void)close(sock_fd);
         return(INCORRECT);
      }
...
wmo_check_reply(void)
{
   ...
   if ((n = readn(sock_fd, buffer, 10)) == 10)
   {
      ...
      /* Show context of what has been returned. */
      {
         ...
         line[line_length] = '\0';
         trans_log(ERROR_SIGN, __FILE__, __LINE__, "%s", line);
      }
   }
        ...
        {
           trans_log(ERROR_SIGN, __FILE__, __LINE__, "select() error : %s",
                     strerror(errno));
        }
        else
        {
   ...
   if (p_data != NULL)
   {
      if (fflush(p_data) == EOF)
      {
         ...
         trans_log(WARN_SIGN, __FILE__, __LINE__,
                   "fflush() error when closing connection : %s",
                   strerror(errno));
      }
