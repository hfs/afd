...
ftp_connect(char *hostname, int port)
{
   ...
   {
      trans_log(ERROR_SIGN, __FILE__, __LINE__,
                "ftp_connect(): fdopen() control error : %s", strerror(errno));
      (void)close(control_fd);
      return(INCORRECT);
   }
...
ftp_keepalive(void)
{
   ...
   if (reply == 0)
   {
      ...
      return(INCORRECT);
        ...
        {
           msg_str[0] = '\0';
           trans_log(ERROR_SIGN, __FILE__, __LINE__, "ftp_keepalive(): Unknown condition.");
           return(INCORRECT);
        }
...
ftp_list(int mode, int type, ...)
{
   ...
   if ((p_data = fdopen(new_sock_fd, "r")) == NULL)
   {
      msg_str[0] = '\0';
      trans_log(ERROR_SIGN, __FILE__, __LINE__,
                "ftp_list(): fdopen() data error : %s", strerror(errno));
      (void)close(new_sock_fd);
      return(INCORRECT);
   }
   ...
   if (fclose(p_data) == EOF)
   {
      msg_str[0] = '\0';
      trans_log(ERROR_SIGN, __FILE__, __LINE__,
                "ftp_list(): fclose() error : %s", strerror(errno));
      return(INCORRECT);
   }
   p_data = NULL;
...
ftp_data(char *filename, off_t seek, int mode, int type)
{
   ...
   else /* ACTIVE_MODE */
   {
      ...
      {
         msg_str[0] = '\0';
         trans_log(ERROR_SIGN, __FILE__, __LINE__, "ftp_data(): accept() timeout");
         timeout_flag = ON;
         (void)close(sock_fd);
         return(INCORRECT);
      }
      ...
      {
         msg_str[0] = '\0';
         trans_log(DEBUG_SIGN, __FILE__, __LINE__, "ftp_data(): close() error : %s",
                   strerror(errno));
      }
   } /* ACTIVE_MODE */
   ...
   {
      msg_str[0] = '\0';
      trans_log(ERROR_SIGN, __FILE__, __LINE__, "ftp_data(): setsockopt() error : %s",
                strerror(errno));
      (void)close(new_sock_fd);
      return(INCORRECT);
   }
   ...
   if ((p_data = fdopen(new_sock_fd, "r+")) == NULL)
   {
      msg_str[0] = '\0';
      trans_log(ERROR_SIGN, __FILE__, __LINE__,
                "ftp_data(): fdopen() data error : %s", strerror(errno));
      (void)close(new_sock_fd);
      return(INCORRECT);
   }
...
ftp_close_data(int type)
{
   ...
   if (type == DATA_WRITE)
   {
      if (fflush(p_data) == EOF)
      {
         msg_str[0] = '\0';
         trans_log(DEBUG_SIGN, __FILE__, __LINE__,
                   "ftp_close_data(): Failed to fflush() data : %s",
                   strerror(errno));
         (void)fclose(p_data);
         p_data = NULL;
         return(INCORRECT);
   ...
   if (fclose(p_data) == EOF)
   {
      msg_str[0] = '\0';
      trans_log(ERROR_SIGN, __FILE__, __LINE__,
                "ftp_close_data(): fclose() error : %s", strerror(errno));
      return(INCORRECT);
   }
   p_data = NULL;
      ...
      return(INCORRECT);
   ...
   else if (FD_ISSET(data_fd, &wset))
        {
           ...
           if ((status = send(data_fd, ptr, size, 0)) != size)
           {
              msg_str[0] = '\0';
              trans_log(ERROR_SIGN, __FILE__, __LINE__,
                        "ftp_write(): send() error (%d) : %s", status, strerror(errno));
              return(errno);
           }
#else
...
ftp_quit(void)
{
   ...
   if (p_data != NULL)
   {
      if (fflush(p_data) == EOF)
      {
         msg_str[0] = '\0';
         trans_log(WARN_SIGN, __FILE__, __LINE__,
                   "ftp_quit(): Failed to fflush() data connection : %s",
                   strerror(errno));
      }
#ifdef _WITH_SHUTDOWN
      if (shutdown(data_fd, 1) < 0)
      ...
      if (fclose(p_data) == EOF)
      {
         msg_str[0] = '\0';
         trans_log(DEBUG_SIGN, __FILE__, __LINE__,
                   "ftp_quit(): fclose() error : %s", strerror(errno));
      }
   }
