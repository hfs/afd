 ...
 **           -p <no. of process>    the maximum number of process
 **                                  for the dir_check
               ...
               int  *max_no_proc)
{
   ...
   /*                                                        */
   /*         -p : The maximum number of process that AMG    */
   /*               may generate for dir_check.              */
   /*                                                        */
   ...
   while ((--argc > 0) && ((*++argv)[0] == '-'))
   {
      switch(*(argv[0] + 1))
      {
         ...
         case 'p': /* Max. no. of process */

            if ((argc == 1) || (*(argv + 1)[0] == '-'))
            {
               (void)fprintf(stderr, "ERROR  : You did not specify the number of process.\n");
               correct = NO;
            }
            else
            {
               *max_no_proc = atoi(*(argv + 1));
               argc--;
               argv++;
            }
            break;
...
usage(void)
{
   ...
   (void)fprintf(stderr, "          -p         The maximum number of process that the AMG may generate for ?????.\n");
