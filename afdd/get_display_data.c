		 ...
		 int  file_no)
{
   ...
   {
      length = sprintf(reply, "500 Failed to open() %s : %s (%s %d)\r\n",
   ...
   {
      length = sprintf(reply, "500 Failed to fstat() %s : %s (%s %d)\r\n",
                       search_file, strerror(errno), __FILE__, __LINE__);
      (void)close(from_fd);
      return(INCORRECT);
   }
   ...
   n = sprintf(buffer, "211- Command successful\r\n");
   (void)write(fd, buffer, n);

   while (left > 0)
   {
      if (read(from_fd, buffer, hunk) != hunk)
      {
         length = sprintf(reply, "500 Failed to read() %s : %s (%s %d)\r\n",
                          search_file, strerror(errno), __FILE__, __LINE__);
         free(buffer);
         (void)close(from_fd);
         return(INCORRECT);
      ...
      {
         length = sprintf(reply, "520 write() error : %s (%s %d)\r\n",
                          strerror(errno), __FILE__, __LINE__);
         free(buffer);
         (void)close(from_fd);
         return(INCORRECT);
   ...
   length = sprintf(reply, "200 End of data\n");
   free(buffer);
   if (close(from_fd) == -1)
   {
