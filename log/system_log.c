...
#include <string.h>                /* strcmp(), strcpy(), strcat(),      */
                                   /* strlen()                           */
#include <stdlib.h>                /* getenv(), exit(), abort()          */
...
#ifdef _FIFO_DEBUG
      log_stat = logger(p_log_file, sys_log_fd, "system_log",
                        SYSTEM_LOG_RESCAN_TIME);
#else
      log_stat = logger(p_log_file, sys_log_fd,  SYSTEM_LOG_RESCAN_TIME);
#endif
