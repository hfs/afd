...
write_header(int fd)
{
   ...
   {
      length = sprintf(buffer, "                                AFD DELETE LOG\n\n\
        ...
        {
           length = strftime(buffer, 1024, "                                AFD DELETE LOG\n\n\tTime Interval : %m.%d. %H:%M",
   ...
   {
      length += sprintf(&buffer[length], "                Delete type   : %s", AGE_OUTPUT_ID_STR);
   }
   if (toggles_set & SHOW_AGE_INPUT)
   {
      if (length == tmp_length)
      {
         length += sprintf(&buffer[length], "                Delete type   : %s", AGE_INPUT_ID_STR);
      }
      else
      {
   ...
   if (toggles_set & SHOW_USER_DEL)
   {
      if (length == tmp_length)
      {
         length += sprintf(&buffer[length], "                Delete type   : %s", USER_DEL_ID_STR);
      }
      else
      {
   ...
   if (toggles_set & SHOW_OTHER_DEL)
   {
      if (length == tmp_length)
      {
         length += sprintf(&buffer[length], "                Delete type   : %s", OTHER_DEL_ID_STR);
      }
      else
      {
