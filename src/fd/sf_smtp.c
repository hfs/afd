...
# endif
   ...
   do
   {
      ...
      if ((burst_2_counter == 0) || (values_changed & AUTH_CHANGED))
      {
         ...
         {
            if ((status = smtp_ehlo(host_name)) != SUCCESS)
            {
               trans_log(ERROR_SIGN, __FILE__, __LINE__, NULL, msg_str,
            ...
            else
            {
               if (fsa->debug > NORMAL_MODE)
               {
                  trans_db_log(INFO_SIGN, __FILE__, __LINE__, NULL, "Send EHLO.");
               }
            }
            ...
            else
            {
               ...
               if (rejected_user == db.no_listed)
               {
                  ...
                  *dl.unique_number = db.unique_number;
                  ...
                  remove_job_files(file_path, db.fsa_pos,
#ifdef _DELETE_LOG
                                   db.job_id, SEND_FILE_SMTP, RECIPIENT_REJECTED,
#endif
                                   db.lock_offset);
                  trans_log(ERROR_SIGN, __FILE__, __LINE__, NULL, NULL,
         ...
         if (((db.special_flag & ATTACH_ALL_FILES) == 0) || (files_send == 0))
         {
            ...
            if (db.special_flag & MAIL_SUBJECT)
            {
               ...
               else
               {
                  ...
                  {
                     if ((length + strlen(final_filename) + 2) < blocksize)
                     {
                  ...
                  {
                     if ((length + strlen(&db.subject[db.filename_pos_subject + 2]) + 2) < blocksize)
                     {
                        length += sprintf(&buffer[length], "%s\r\n",
