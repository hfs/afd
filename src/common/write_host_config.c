...
#                ===================================\n\
#\n\
# There are 17 parameters that can be configured for each remote\n\
# host. They are:\n\
#\n\
...
#\n\
# Example: idefix:192.168.1.24:192.168.1.25:[12]::5:10:300:4096:10:0:0:1:0:0:0\n\n"
                  ...
                  struct host_list *p_hl)
{
   ...
   for (i = 0; i < no_of_hosts; i++)
   {
      length = sprintf(line_buffer,
                       "%s:%s:%s:%s:%s:%d:%d:%d:%d:%d:%d:%d:%d:%d:%u:%d:%d\n",
                       p_hl[i].host_alias,
