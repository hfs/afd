/*
 *  save_old_year.c - Part of AFD, an automatic file distribution program.
...
/*
 ** NAME
 **   save_old_year - saves all information of one year into a
 **                   separate file
 **
 ** SYNOPSIS
 **   void save_old_year(int new_year)
 **
 ** DESCRIPTION
 **
 ** RETURN VALUES
...
save_old_year(int new_year)
{
   ...
   (void)rec(sys_log_fd, INFO_SIGN, "Saving statistics for year %d (%s %d)\n",
   ...
   old_year_stat_size = no_of_hosts * sizeof(struct afd_year_stat);
   ...
   if ((old_stat_db = (struct afd_year_stat *)mmap(0, old_year_stat_size,
                                                   (PROT_READ | PROT_WRITE),
                                                   (MAP_FILE | MAP_SHARED),
                                                   old_year_fd, 0)) == (struct afd_year_stat *) -1)
   {
      (void)rec(sys_log_fd, ERROR_SIGN,
...
#ifdef _NO_MMAP
   if (write(old_year_fd, old_stat_db, old_year_stat_size) != old_year_stat_size)
   {
      (void)rec(sys_log_fd, ERROR_SIGN,
                "Could not write() to %s : %s (%s %d)\n",
                statistic_file, strerror(errno), __FILE__, __LINE__);
   ...
   free(old_stat_db);
#else
   if (munmap((void *)old_stat_db, old_year_stat_size) == -1)
   {
      (void)rec(sys_log_fd, ERROR_SIGN,
